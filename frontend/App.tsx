...
import { PendenciaBadges, temPendencia } from './components/ui/PendenciaBadges';
...
export default function App() {
    ...
    const getStatData = (type: string | null, list: UserType[]) => {
        ...
        switch(type) {
            ...
            case 'PENDENTES': return list.filter(temPendencia);
    ...
    /**
     * Ordena uma lista de pessoas conforme o seletor "Ordenar por".
     *
     * Sempre devolve um array NOVO — as listas de origem vem de useMemo e
     * ordenar no lugar mutaria o valor memorizado.
     *
     * Quem nao tem supervisao vai para o fim (e nao junto do "N/A" no meio do
     * alfabeto), porque e um caso de excecao que interessa ver agrupado.
     */
    const ordenarLista = (lista: UserType[]): UserType[] => {
        const ordemPosicao: Record<Role, number> = { ADM: 0, PASTOR: 1, DISCIPULADOR: 2, DISCIPULO: 3 };
        const porNome = (a: UserType, b: UserType) => a.name.localeCompare(b.name, 'pt-BR');
        // null = sem supervisao (vai para o fim da lista)
        const chaveSupervisao = (u: UserType): string | null => {
            const id = u.discipuladorId || u.pastorId;
            const sup = id ? usersDb.find(s => s.id === id) : undefined;
            return sup ? sup.name.toLocaleLowerCase('pt-BR') : null;
        };

        return [...lista].sort((a, b) => {
            if (sortBy === 'nome') return porNome(a, b);
            if (sortBy === 'posicao') {
                const diff = (ordemPosicao[a.role] ?? 99) - (ordemPosicao[b.role] ?? 99);
                return diff !== 0 ? diff : porNome(a, b);
            }
            const supA = chaveSupervisao(a);
            const supB = chaveSupervisao(b);
            if (supA === null || supB === null) {
                if (supA === supB) return porNome(a, b);
                return supA === null ? 1 : -1;
            }
            const diff = supA.localeCompare(supB, 'pt-BR');
            return diff !== 0 ? diff : porNome(a, b);
        });
    };

    /**
     * Seletor de ordenacao usado nas quatro listas.
     *
     * Fundo explicito no select e nos <option>: a lista suspensa nativa herda a
     * cor de fundo do select, e com bg-transparent o popup abre branco com
     * texto branco no modo escuro.
     */
    const opcaoClasse = 'bg-white text-gray-700 dark:bg-slate-700 dark:text-white';
    const SortSelect = () => (
        <div className="bg-white dark:bg-slate-700 flex items-center gap-3 px-5 py-3 rounded-2xl border border-gray-100 dark:border-slate-600 w-full md:w-auto shadow-sm">
            <ListFilter size={18} className="text-gray-400 shrink-0" />
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest shrink-0 hidden lg:inline">{t.common.sortBy}</label>
            <select
                value={sortBy}
                onChange={(e) => setSortBy(e.target.value as typeof sortBy)}
                aria-label={t.common.sortBy}
                className="bg-white dark:bg-slate-700 outline-none text-sm font-medium text-gray-700 dark:text-white w-full md:w-auto cursor-pointer"
            >
                <option className={opcaoClasse} value="nome">{t.users.name}</option>
                <option className={opcaoClasse} value="posicao">{t.common.position}</option>
                <option className={opcaoClasse} value="supervisao">{t.common.supervision}</option>
            </select>
        </div>
    );
                ...
                <main className="flex-1 p-4 lg:p-10 h-dvh overflow-y-auto scroll-smooth">
                    ...
                    {view === 'dashboard' && (
                        <div className="space-y-8 md:space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 md:pb-0">
                            ...
                            <div><h3 className="text-base md:text-lg font-bold text-gray-800 dark:text-white mb-4 md:mb-5 flex items-center gap-2"><span className="w-2 h-6 bg-teal-500 rounded-full inline-block"></span>{t.dashboard.networkStatus}</h3>
                                <div className="flex flex-col gap-4 md:gap-6">
                                    <div className="bg-white dark:bg-slate-700 flex items-center gap-3 px-5 py-3 rounded-2xl border border-gray-100 dark:border-slate-600 w-full md:max-w-sm shadow-sm">
                                        ...
                                        {/* Fundo explicito no select (e nao bg-transparent): a lista suspensa
                                            nativa herda a cor de fundo do proprio select. Com fundo
                                            transparente + dark:text-white, o popup abria branco com texto
                                            branco — invisivel. As classes nos <option> garantem o contraste
                                            tambem nos navegadores que estilizam a lista separadamente. */}
                                        <select value={filterMinisterio ?? ''} onChange={(e) => setFilterMinisterio(e.target.value || null)} className="bg-white dark:bg-slate-700 outline-none text-sm font-medium text-gray-700 dark:text-white w-full cursor-pointer">
                                            <option className="bg-white text-gray-700 dark:bg-slate-700 dark:text-white" value="">{t.dashboard.allMinistries}</option>
                                            <option className="bg-white text-gray-700 dark:bg-slate-700 dark:text-white" value="__SEM__">{t.dashboard.withoutMinistry}</option>
                                            {ministeriosDisponiveis.map(m => <option className="bg-white text-gray-700 dark:bg-slate-700 dark:text-white" key={m.chave} value={m.chave}>{m.label}</option>)}
                                        </select>
                                    </div>
                            ...
                            <div className="bg-white dark:bg-slate-800 rounded-[1.5rem] md:rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden transition-all duration-500"><div className="p-6 md:p-8 border-b border-gray-100 dark:border-slate-700 flex flex-wrap justify-between items-center gap-4"><div className="flex items-center gap-3"><div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl text-blue-600 dark:text-blue-400"><ListFilter size={24} /></div><div><h3 className="font-bold text-gray-800 dark:text-white text-base md:text-lg">{selectedStat ? `${t.dashboard.details}: ${selectedStat}` : t.dashboard.memberList}</h3><p className="text-xs text-gray-500 dark:text-gray-400">{shouldShowList ? `${t.dashboard.showingPeople} ${statDetailsList.length} ${t.dashboard.peopleWithFilters}` : t.dashboard.clickCardMessage}</p></div></div><SortSelect />{(selectedStat || filterGender || filterAge || searchTerm) && (<button onClick={() => { setSelectedStat(null); setFilterGender(null); setFilterAge(null); setFilterMinisterio(null); setSearchTerm(''); }} className="text-sm text-gray-400 hover:text-red-500 transition-colors flex items-center gap-1 font-medium"><X size={16} /> {t.dashboard.clearAll}</button>)}</div><div className="overflow-x-auto max-h-[400px] overflow-y-auto">{shouldShowList ? (statDetailsList.length > 0 ? (<table className="w-full text-left"><thead className="bg-gray-50/50 dark:bg-slate-700/50 text-gray-400 text-xs uppercase font-bold tracking-widest sticky top-0 z-10 backdrop-blur-sm"><tr><th className="px-6 py-4">{t.users.name}</th><th className="px-6 py-4">{t.common.position}</th><th className="px-6 py-4">{t.common.supervision}</th><th className="px-6 py-4 text-right">{t.common.action}</th></tr></thead><tbody className="divide-y divide-gray-50 dark:divide-slate-700">{ordenarLista(statDetailsList).map((item) => (<tr key={item.id} className="hover:bg-blue-50/10 dark:hover:bg-slate-700/30 transition"><td className="px-6 py-3"><div className="flex items-center gap-3"><AvatarPlaceholder name={item.name} size="sm" /><div><span className="font-medium text-sm text-gray-700 dark:text-gray-200 block">{item.name}</span>{item.nascimento && <span className="text-[10px] text-gray-400">{calculateAge(item.nascimento)} {t.users.years}</span>}<PendenciaBadges item={item} /></div></div></td><td className="px-6 py-3"><Badge type={item.role}>{item.role}</Badge></td><td className="px-6 py-3 text-sm text-gray-500 dark:text-gray-400">{getSupervisorName(item.discipuladorId || item.pastorId)}</td><td className="px-6 py-3 text-right"><div className="flex justify-end items-center gap-2"><WhatsAppButton contato={item.contato} nome={item.name} tamanho="sm" /><button onClick={() => openForm(item.role, item)} className="p-1.5 text-gray-400 hover:text-blue-500 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition"><Edit size={16} /></button></div></td></tr>))}</tbody></table>) : (<div className="p-10 text-center text-gray-400"><p>{t.dashboard.noPersonFound}</p></div>)) : (<div className="p-12 flex flex-col items-center justify-center text-gray-400 opacity-60"><Activity size={48} className="mb-4 text-gray-300 dark:text-slate-600" /><p className="text-sm font-medium">{t.dashboard.selectStatMessage}</p></div>)}</div></div>
                        </div>
                    )}
                    ...
                    {(view === 'disciples') && (
                        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
                            ...
                            <div className="bg-white dark:bg-slate-800 rounded-[1.5rem] md:rounded-[2.5rem] border border-gray-100 dark:border-slate-700 shadow-xl shadow-gray-100/50 dark:shadow-none overflow-hidden">
                                <div className="p-4 md:p-6 border-b border-gray-100 dark:border-slate-700 flex flex-col md:flex-row items-center gap-4 bg-gray-50/30 dark:bg-slate-700/30"><div className="bg-white dark:bg-slate-700 flex items-center gap-3 px-5 py-3 rounded-2xl border border-gray-100 dark:border-slate-600 w-full md:max-w-sm shadow-sm focus-within:ring-2 focus-within:ring-blue-100 transition-all"><Search size={18} className="text-gray-400 shrink-0" /><input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder={t.common.filterByName} className="bg-transparent outline-none text-sm font-medium text-gray-700 dark:text-white w-full placeholder-gray-400" /></div><SortSelect /></div>
                                <div className="hidden md:block overflow-x-auto"><table className="w-full text-left"><thead className="bg-gray-50/50 dark:bg-slate-700/50 text-gray-400 text-xs uppercase font-bold tracking-widest"><tr><th className="px-8 py-6">{t.users.name}</th><th className="px-6 py-6">{t.common.position}</th><th className="px-6 py-6">{t.common.supervision}</th><th className="px-8 py-6 text-right">{t.common.actions}</th></tr></thead><tbody className="divide-y divide-gray-50 dark:divide-slate-700">{ordenarLista(disciplesList).map((item) => (<tr key={item.id} className="group hover:bg-blue-50/30 dark:hover:bg-slate-700/50 transition"><td className="px-8 py-5"><div className="flex items-center gap-5"><AvatarPlaceholder name={item.name} size="md" /><div><p className="font-bold text-gray-800 dark:text-white text-sm">{item.name}</p><p className="text-xs text-gray-400">{item.email || t.users.noEmail}</p><PendenciaBadges item={item} /></div></div></td><td className="px-6 py-5"><Badge type={item.role}>{item.role}</Badge></td><td className="px-6 py-5 text-sm text-gray-500 dark:text-gray-400">{getSupervisorName(item.discipuladorId || item.pastorId)}</td><td className="px-8 py-5 text-right"><div className="flex justify-end gap-2"><WhatsAppButton contato={item.contato} nome={item.name} /><button onClick={() => openForm(item.role, item)} className="p-2.5 text-gray-400 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 bg-white dark:bg-slate-700 hover:shadow-md rounded-xl transition" aria-label={`Editar ${item.name}`}><Edit size={18} /></button></div></td></tr>))}</tbody></table></div>
                                <div className="md:hidden p-4 bg-gray-50/30 dark:bg-slate-900/30">{ordenarLista(disciplesList).map((item) => (<MobileUserCard key={item.id} item={item} view={view} openForm={openForm} handleDelete={handleDelete} user={user} getSupervisorName={getSupervisorName} onOpenCell={() => { }} />))}</div>
                            </div>
                        </div>
                    )}
                </main>
